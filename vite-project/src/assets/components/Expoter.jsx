import React, { Suspense, memo, useMemo, useEffect, useState } from "react";
import { 
  ContactShadows,
  Environment,
  useTexture,
  Html,
  Text
} from "@react-three/drei";
import Model from "./Avatar";
import { Page, SkillsPage, ProjectsPage, ContactsPage } from "./Pages";
import { Dragon } from "./Homemodel";
import { Skills3DBox } from "./skillpage";
import { Flower } from "./Flower";
import { Butterfly } from "./Butterfly";
import { FireText } from "./Firetext";
import { Contact3D } from "./Contect";
 
import { Monitor } from "./Monitor";
import { FloatingProjects } from "./Projects";



// Mobile detection utility
const isMobile = () => {
  if (typeof window === 'undefined') return false;
  return window.innerWidth <= 768 || /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
};

// Large Floor component
const Floor = memo(() => {
  const floorTexture = useTexture('data:image/jpeg;base64,');
  
  // Configure texture properties for better tiling
  floorTexture.wrapS = floorTexture.wrapT = 1000; // RepeatWrapping
  floorTexture.repeat.set(50, 50); // Large repetition for big floor
  
  return (
    <mesh position-y={-0.001} rotation-x={-Math.PI/2}> 
      <planeGeometry args={[200, 200]} />
      <meshStandardMaterial
        map={floorTexture}
        roughness={0.8}
        metalness={0.1}
      />
    </mesh>
  );
});

Floor.displayName = 'Floor';

// Memoized components for better performance
const MemoizedModel = memo(Model);
const MemoizedDragon = memo(Dragon);
const MemoizedSkills3DBox = memo(Skills3DBox);
const MemoizedFlower = memo(Flower);
const MemoizedButterfly = memo(Butterfly);
const MemoizedContact3D = memo(Contact3D);


export const Expoter = memo(({ dark, isMobile: propIsMobile }) => {
  const [mobile, setMobile] = useState(false);
  
  useEffect(() => {
    setMobile(propIsMobile || isMobile());
  }, [propIsMobile]);

  // Memoize static props with mobile considerations
  const environmentPreset = useMemo(() => "city", []);
  const shadowProps = useMemo(() => ({ 
    opacity: mobile ? 1 : 1, 
    scale: mobile ? [10, 10] : [10, 10], 
    color: "#000000" 
  }), [mobile]);
  
  const fireTextProps = useMemo(() => ({
    aboutMe: {
      text: "About Me!",
      position: mobile ? [-4, 3, -15] : [-7, 4, -25],
      size: mobile ? 0.4 : 0.7
    },
    description: {
      text: mobile ? "Fullstack Developer" : "I am Aniket Jha, a passionate fullstack developer.",
      position: mobile ? [-4, 2, -15] : [-7, 2, -25],
      size: mobile ? 0.3 : 0.5
    }
  }), [mobile]);

  return (
    <>
      <Environment preset={environmentPreset} />
      
      {/* Large Floor */}
      <Floor />
      
      {/* Core Model with responsive scaling */}
      <Suspense fallback={null}>
        <group scale={mobile ? 1 : 1}>
          <MemoizedModel isMobile={mobile} />
        </group>
      </Suspense>

      {/* Pages - Essential content */}
      <Page dark={dark} />
      <SkillsPage dark={dark} />
      <ProjectsPage dark={dark} />
      <ContactsPage dark={dark} />
       
      <Monitor/>

      {/* 3D Components with mobile scaling */}
      <Suspense fallback={null}>
        <group scale={mobile ? 0.6 : 1}>
          <MemoizedSkills3DBox />
        </group>
        
        {/* Dragon and Flower - now visible on both mobile and desktop */}
        <group scale={mobile ? 1.2 : 1.2}>
          <MemoizedDragon />
          <MemoizedFlower />
        </group>
        
        <group scale={mobile ? 0.7 : 1}>
          <MemoizedButterfly dark={dark} />
        </group>
      </Suspense>

      {/* Text Elements with mobile adjustments */}
      <FireText {...fireTextProps.aboutMe} />
      <FireText {...fireTextProps.description} />
      
      {/* Contact Section with mobile scaling */}
      <group scale={mobile ? 0.8 : 1}>
        <MemoizedContact3D />
      </group>
      
      {/* Floating Projects */}
      <FloatingProjects dark={dark} />
    </>
  );
});

Expoter.displayName = 'Expoter';






